...
def profile():
    ...
    is_update_ok = formdb.set_db_user_password(formdb.get_db_user_id(), password_new)
    if is_update_ok:
...
def update_record(collection_name, record_id):
    ...
    if not record:
        flash(f"{formdb.get_db_entity_name(collection_name)} {record_id} does not exist", "danger")
        return redirect(url_for('maintain', collection_name=collection_name))
...
def delete_record(collection_name, record_id):
    ...
    if not record:
        ...
        flash(f"Deleted one {formdb.get_db_entity_name(collection_name)} record", "info")
    return redirect(url_for('maintain', collection_name=collection_name))
